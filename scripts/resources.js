...
var Resources = function () {
  ...
  /* FON :: Use it to refactor */
  this.preloadAsset = function (asset) {
    this.list['player'] = {};
    Object.keys(this[asset]).forEach((k) => {
      if (k !== 'path') {
        self.list[asset][k] = new Asset();
        self.list[asset][k].loadImage(`${self[asset].path}${self[asset][k]}`);
      }
    });
  }
